...
contract MockingBirdToken is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 MBird-WAVAX LP -> MBird pool
    uint256 public constant INITIAL_MBIRD_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the Mbird ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("MockingBird Token", "MBIRD") {
        // Mints 1 MBIRD to contract creator for initial pool setup
    ...
    function _getMbirdPrice() internal view returns (uint256 _MbirdPrice) {
        try IOracle(MbirdOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Mbird: failed to fetch Mbird price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _MbirdPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_MbirdPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMbirdOracle(address _MbirdOracle) public onlyOperatorOrTaxOffice {
        require(_MbirdOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints MBIRD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MBIRD to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMbirdPrice = _getMbirdPrice();
            currentTaxRate = _updateTaxRate(currentMbirdPrice);
            if (currentMbirdPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_MbirdPool != address(0), "!_MbirdPool");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_MbirdPool, INITIAL_MBIRD_POOL_DISTRIBUTION);
    }
